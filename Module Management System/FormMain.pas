...
uses
  ...
  mTypes, GetKHAversion, FormSetting;
...
begin
  ...
  Try
    ...
    begin
      ...
      begin
        aParam := MainForm.lvModule.Items.Item[dclI].SubItems[4];
        // varify value o
        aParam := seedDcpFromSHA256('mParam', aParam);
        aParam := StringReplace(aParam, '[DTP-START]',
          FormatDateTime('yyyymmdd', MainForm.dtpFrom.DateTime),
          [rfReplaceAll, rfIgnoreCase]);
        aParam := StringReplace(aParam, '[DTP-END]', FormatDateTime('yyyymmdd',
          MainForm.dtpTo.DateTime), [rfReplaceAll, rfIgnoreCase]);
        mExecute := '..\module\' + MainForm.lvModule.Items.Item[dclI].SubItems
          [3] + '.kha';
        // Check mExecute File exist
        if FileExists(mExecute) then
        begin
          MainForm.lvModule.Items.Item[dclI].SubItems[6] :=
            _GetKHAFileData(mExecute).FileVersion;
          mExecute := mExecute + ' "' + aParam + '"';
